...
const SearchUsingApi = ({navigation, route}) => {
...
//   }, []);
  ...
  //load Data
   ...
   const renderItem = (row, index) => {
      return (
       <ProductTableRow 
            img={row.img_url }name={row.name}code={row.product_code} price={(priceShow ==="Giá bán"? row.list_price:row.standard_price).toLocaleString()}branch_quantity={row.branch_quantity} uuid={row.uuid}handleOnPress={()=>handleSearchBarSelect(row)}/>
      );
