...
const ModalImportSummary = ({selectedIndex,showModal,setShowModal,cartData,total_amount,total_quantity,navigation,suppliers,handleSelectSupplier,handleDelete}) => {
...
const handleConfirm = async() =>{
    ...
    let body = {
      supplier_uuid: cartData.supplier ? cartData.supplier.uuid : "",
        ...
        <Modal.Content maxWidth="350">
          ...
          <Modal.Body>
            <HStack >
              <Input 
                  ...
                  h={10}
                  size="sm" p={1} variant="rounded" placeholder="Tìm nhà cung cấp ..."  
         ...
         <PresenceTransition visible={value.length !== 0} initial={{ opacity: 0  }} animate={{ opacity: 1, transition: { duration: 250  } }}>
            ...
            <VStack space={3}  mt={5}>
              ...
              <HStack alignItems="center" justifyContent="space-between">
                <Text fontWeight="medium">Trả NCC</Text>
              ...
              <HStack alignItems="center" justifyContent="space-between">
                <Text fontWeight="medium">Còn lại</Text>
                <Text color="blueGray.400">{(Number(total_amount)- Number(discount) - Number(paidAmmount)).toLocaleString()}</Text>
              </HStack>
              </VStack>
              ...
              <Button mt={6} colorScheme="secondary" onPress={handleConfirm} >Nhập hàng</Button>
...
export default ModalImportSummary