...
const InvoiceDetailScreen = ({navigation, route}) => {
  ...
  return (
   ...
   <>
    <BackNavBar navigation={navigation} title={"Chi tiết hoá đơn"}  > 
        {/*  */}
     ...
     */}
      <HStack  alignItems='center'>
        ...
        <Text fontSize={16} >{row.order_code}</Text>
      </HStack> 
      <HStack>
      ...
      <HStack>
        ...
        <Text fontSize={16} >{row.customer ?row.customer:""}</Text>
      </HStack> 
      <HStack>
        <Text bold w="50%"color='black' >Người bán : </Text>
      ...
      <PopUpPayRemaining 
          ...
          title={`Thu thêm hoá đơn ${row.order_code}` }
